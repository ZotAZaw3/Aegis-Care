...
- **AI Agent** — advises with cited data, judges orders against the KB + record, flags gaps; the doctor decides. Realized as the copilot orchestrator (`/api/copilot`) and the deterministic violation engine (`order_violations`). Management dashboards exist; a live "AI Meeting Copilot" for briefings is a stated goal, only partly built — do not assume unbuilt features exist; verify in code.
